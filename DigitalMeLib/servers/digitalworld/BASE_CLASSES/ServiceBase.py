...
def action(func):
    def wrapper_action(*args, **kwargs):
        ...
        if skip_for_debug or "noqueue" in kwargs:
            ...
            res = msgpack.loads(action.result)
            self.logger.debug("METHOD EXECUTED OK")
            return action
...
class ServiceBase(JSBase):
    ...
    def _action_new(self,name,args,kwargs):
        ...
        args2 = msgpack.dumps(args)
        kwargs2 = msgpack.dumps(kwargs)
    ...
    def _main(self):
        ...
        while True:
            ...
            self.logger.info("action execute:\n%s"%a)
            a.time_start = j.data.time.epoch
            res = func(self,*args,**kwargs)
            print("main res:%s"%res)
            a.result= msgpack.dumps(res)
            a.save()
            event.set()
